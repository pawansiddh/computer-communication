...
#include <netdb.h>
#include <arpa/inet.h>
#include "tictac.h" #include <unistd.h> // for sleep()
#include <sys/socket.h> // for close()
#define PORT "9987" using namespace std;
...
if (bytes_recvd == -1)
cout<<"COULD NOT ACQUIRE PLAYERINFORMATION!"<<endl<<"Trying Again..."<<endl;
else
...
cout<<sname<<" has chosen "<<serv_choice<<endl<<endl<<"Youwill play with "<<cli_choice<<endl;
...
cli_choice = 'X';
inp_true = 1;
cout<<endl<<sname<<" gets O."<<endl<<endl<<"LetsPlay!"<<endl<<endl;
}
...
cli_choice = 'O';
inp_true = 1;
cout<<endl<<sname<<" gets X."<<endl<<endl<<"Lets
Play!"<<endl<<endl;
}
else
{
...
{
inp ++;
cout<<endl<<sname<<" has played. Updating
Matrix..."<<endl;
17
}
}
...
if (bytes_sent == -1)
{
perror("CO-ORDINATES BUFFERcouldnot besent!");
return 1;
}
}
}
...
{
cout<<endl<<"You loose."<<endl<<sname<<" has
won."<<endl;
break;
}
else if (cli_choice == nc)
{
cout<<endl<<"Congrats! You have
won!!!"<<endl<<sname<<" lost."<<endl;
break;
}
}
}
