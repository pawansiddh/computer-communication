...
#include <arpa/inet.h>
#include <unistd.h>
#include "tictac.h" #define PORT "9987";
using namespace std;
...
{
memset(&cname, 0, sizeof(cname));
cout<<"Could not ACQUIRE Player Information!"<<endl<<"Tryingagain..."<<endl;
continue;
}
else
{
...
if (bytes_sent == -1)
cout<<"Could not SEND Player Data!"<<"TryingAgain..."<<endl;
else
...
cli_choice = 'O';
inp_true = 1;
cout<<endl<<cname<<" gets O."<<endl<<endl<<"LetsPlay!"<<endl<<endl;
}
else if (serv_choice == 'O' || serv_choice == 'o' || serv_choice =='0'){
serv_choice = 'O';
cli_choice = 'X';
inp_true = 1;
cout<<endl<<cname<<" gets X."<<endl<<endl<<"Lets
Play!"<<endl<<endl;
}
else
{
...
if (bytes_sent == -1)
{
perror("CO-ORDINATES BUFFERnot sent!");
return 1;
}
}
}
...
{
inp ++;
cout<<endl<<cname<<" has played. Updating
Matrix..."<<endl;
}
}
count ++;
...
won!!!"<<endl<<cname<<" lost."<<endl;
break;
}
else if (cli_choice == nc)
{
cout<<endl<<"You loose."<<endl<<cname<<" has
won."<<endl;
break;
}
}
